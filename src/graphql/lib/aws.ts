 ...
 */
export const uploadFileToSupabaseS3 = async (fileName: string, buffer: Buffer): Promise<string | null> => {
    try {
      // Upload the buffer to Supabase storage
      const { data, error } = await supabase.storage
        .from('legitemfiles') // Replace with your actual bucket name
        .upload(fileName, buffer, {
          contentType: 'image/webp', // Make sure to adjust the content type according to your file format
